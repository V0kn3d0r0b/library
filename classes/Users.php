...
<strong>Error !</strong> Email already Exists, please try another Email... !</div>';
  ...
  <strong>Error !</strong> Something went Wrong !</div>';
    ...
    public function addNewBookByAdmin($data){
        $book_title = $data['book_title'];
        $author = $data['author'];
        $categoryid = $data['categoryid'];
        $pdftmp = $_FILES['pdfs']['tmp_name'];
        $pdfname = $_FILES['pdfs']['name'];
        $path = "books/";
        $pdf_path = $path.time().$pdfname;
        $imgtmp = $_FILES['cover_img']['tmp_name'];
        $imgname = $_FILES['cover_img']['name'];
        $imgpath = "img/";
        $img_path = $imgpath.time().$imgname;



        if ($book_title == "" || $author == "" || $categoryid == "") {
            $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Input fields must not be Empty !</div>';
            return $msg;
        }elseif (strlen($book_title) < 3) {
            $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> A book title is too short, at least 3 Characters !</div>';
            return $msg;
        }elseif (strlen($author) < 3) {
            $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Author name is too short, at least 3 Characters !</div>';
            return $msg;

        } else{

            $sql = "INSERT INTO tbl_books(book_title, author, categoryid, pdf_path, img_path) 
                VALUES(:book_title, :author, :categoryid, :pdf_path, :img_path)";
            $stmt = $this->db->pdo->prepare($sql);
            $stmt->bindValue(':book_title', $book_title);
            $stmt->bindValue(':author', $author);
            $stmt->bindValue(':categoryid', $categoryid);
            $stmt->bindValue(':pdf_path', $pdf_path);
            $stmt->bindValue(':img_path', $img_path);
            $result = $stmt->execute();
            move_uploaded_file($pdftmp, $pdf_path);
            move_uploaded_file($imgtmp, $img_path);
            if ($result) {
                $msg = '<div class ="alert alert-success alert-dismissible mt-3" id="flash-msg">
  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  <strong>Success !</strong> Wow, you have Registered a Book Successfully !</div>';
                return $msg;
            }else{
                $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  ...
  public function selectAllUserData(){
    ...
    // Select All Book Method
    public function selectAllBookData(){
...
//Get Single Book Information By Id Method
    public function updateBookByIdInfo($bookid, $data){
        $book_title = $data['book_title'];
        $author = $data['author'];
        $categoryid = $data['categoryid'];
  ...
  <strong>Error !</strong> Input Fields must not be Empty !</div>';
            ...
            $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
    <strong>Error !</strong> Book title is too short, at least 3 Characters !</div>';
            return $msg;
        }else{
    ...
    public function deleteBookById($remove){
        ...
        if ($result) {
            $msg = '<div class="alert alert-success alert-dismissible mt-3" id="flash-msg">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
    <strong>Success !</strong> Book Deleted Successfully !</div>';
            return $msg;
        }else{
            $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
